...
class DeliveryStatusController {
  ...
  async update(req, res) {
    ...
    if (isBefore(endDate, startDate)) {
      return res
        .status(400)
        .json({ error: 'The delivery date must be after the withdrawal date' });
    }
    ...
    if (!deliveryBelongsToDeliveryman) {
      return res.status(401).json({
        error: 'This Delivery is not from this Deliveryman',
      });
    }
    ...
    return res
      .status(401)
      .json({ error: 'The limit is 5 per day, come back later!' });
  }
}
